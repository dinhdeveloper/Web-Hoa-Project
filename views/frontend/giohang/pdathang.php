...
?>
    <form action="index.php?c=7&id=<?php echo $maDonDatHang ?>" method="post" style="text-align: center;background-color: white">
        <h4>Tên Khách Hàng: <?php echo $hoten ?></h4>
        <h4>Địa Chỉ: <?php echo $diachi ?></h4>
        <h4>Số Điện Thoại: <?php echo $sodienthoai ?></h4>
		<?php
			$sql = "SELECT * FROM chitietdondathang WHERE MaDonDatHang = '$maDonDatHang'";
			$result = DataProvider ::ExecuteQuery ($sql);
			while ($row = mysqli_fetch_array ($result)) {
				?>
                <h4>Sản Phẩm: <?php echo $tensanpham ?></h4> <h4>Giá bán: <?php echo number_format ($gia) ?></h4>
                <h4>Số Lượng: <?php echo $soLuong ?></h4>
				<?php
			}
		?>
